...
class Paginator extends Component {
  ...
  createButton(number, title, className, disabled = false) {
    return (
      <button 
        ...
        className={ `${ className } ${ this.props.classes.controlButton } ${ (this.state.activePage === number) ? 'active' : '' }` }
  ...
  createFirstPageButton() {
    return this.createButton(1, this.props.firstPage, 'page first-page');
  }

  createLastPageButton() {
    return this.createButton(this.state.pageCount, this.props.lastPage, 'page last-page' );
  }

  createPrevPageButton() {
    const number = this.state.activePage - 1
    return this.createButton(number, this.props.prevPage, 'page prev-page', !(number > 0))
  }

  createNextPageButton() {
    const number = this.state.activePage + 1;
    return this.createButton(number, this.props.nextPage, 'page next-page', number > this.state.pageCount);
  }
  ...
  render () {
    ...
    const { className, list, createComponent, numberOfItemsForPage, classes } = this.props;
